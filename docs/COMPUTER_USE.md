...
The model-facing failure envelope makes the same fact explicit as `recovery_kind=reobserve`. When the failed request proves the complete target, `suggested_call={tool, arguments}` supplies a parser-ready observation using its exact `client_id`: `stale_surface -> computer_list_windows`, `stale_application -> computer_list_applications`, and `stale_display -> computer_list_displays`. A stale or spent snapshot generation can suggest `computer_snapshot_display` with the original `display_id`, without reusing the spent generation. When the original finder filters or complete target are unavailable, only a non-actionable `reconcile_with` family hint is returned, such as `computer_find_elements` for `stale_element`. These two shapes are mutually exclusive. This metadata is advisory only; it does not refresh an opaque identity or grant effect retry authority.
