...
export function formatCommunicationAvailability(
...
export function formatAgentCardRevision(agent: any, language?: RuntimeLanguage): string {
  return (
    (language === "zh-CN" ? "配置版本 " : "Profile revision ") +
...
export function formatAgentWakeStatus(agent: any, language?: RuntimeLanguage): string {
  const unresolvedWakeCount = Number(agent?.unresolved_wake_count || 0);
  const latestWakeState = String(agent?.latest_wake_state || "none");
  return (
    localizedCountLabel(unresolvedWakeCount, "unresolved Wake", "unresolved Wakes", language) +
...
export function formatAgentEndpointStatus(endpoint: any, language?: RuntimeLanguage): string {
  if (!endpoint) {
  ...
  return (
    (language === "zh-CN" ? "浏览器端点 " : "Browser Endpoint ") +
...
export function formatConversationSeq(
  summary: any,
  detail: any,
  language?: RuntimeLanguage,
): string {
  return (
    (language === "zh-CN" ? "序号 " : "seq ") +
...
export interface ValidatedAgentCreate {
  handle: string;
  displayName: string;
  description: string;
  labels: string[];
...
export function validateAgentCreateInputs(
  handle: string,
  displayName: string,
  description: string,
  labelsRaw: string,
): { valid: true; data: ValidatedAgentCreate } | { valid: false; error: string } {
...
export interface ValidatedAgentUpdate {
  handle: string;
  displayName: string;
  description: string;
...
export function validateAgentUpdateInputs(
  handle: string,
  displayName: string,
  description: string,
  labelsRaw: string,
): { valid: true; data: ValidatedAgentUpdate } | { valid: false; error: string } {
  const cleanHandle = handle.trim();
  const cleanName = displayName.trim();
  const cleanDescription = description.trim();
...
export interface ValidatedConversationCreate {
  title: string;
  agentIds: string[];
  fingerprint: string;
}

export function validateConversationCreateInputs(
  title: string,
  agentIdsRaw: string,
  defaultAgentId = "",
): { valid: true; data: ValidatedConversationCreate } | { valid: false; error: string } {
