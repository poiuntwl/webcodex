...
export function messageLineStartsBlock(line) {
    return /^```/.test(line)
        || /^#{1,3}\s+/.test(line)
        || /^>\s?/.test(line)
        || /^\s*[-*+]\s+/.test(line)
...
export function appendMessageParagraph(parent, lines) {
    if (!lines.length)
        return;
...
export function appendMessageCode(parent, language, codeText, ctx) {
...
export function appendRichMessage(bubble, sourceValue, ctx) {
