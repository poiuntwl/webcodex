    ...
    async function request(full) {
        ...
        try {
            ...
            if (valid) {
                ...
                const nextCursor = typeof response.data.event_cursor === "number" ? response.data.event_cursor : cursor;
...
function renderDetail(d, preserveScroll) {
    ...
    setText("detail-exec-status", "exec: " + (execution && execution.execution_status ? execution.execution_status : "not available"));
    ...
    setText("detail-recipe", recipe
        ? [recipe.id, recipe.version, recipe.root].filter((value) => !!value).join(" · ")
        : "not available");
    const evidence = (validation && validation.assertion_evidence) ||
...
function renderTimeline(d) {
    ...
    for (const event of [...events].reverse()) {
        ...
        head.appendChild(meta);
        item.appendChild(head);
        const guidance = event && event.kind === "human_guidance" && event.payload
...
function openConfirmUi(action) {
    ...
    setText("confirm-title", action === "accept" ? "Accept result" : action === "reject" ? "Reject result" : "Cancel task");
    ...
    if (body) {
        ...
        const files = review.result && Array.isArray(review.result.changed_paths)
            ? review.result.changed_paths.length
            : 0;
        addLine(body, "Changed files", String(files));
        const validation = review.result && review.result.validation && review.result.validation.status
    ...
    const reasonAvailable = action === "cancel" ||
...
function renderConnect(data) {
    const base = typeof data.public_url === "string" && data.public_url
        ? data.public_url
...
async function decideApproval(row, approve, reason) {
...
function setActivityClientFilter(client) {
