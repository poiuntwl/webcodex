...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Run one bounded shell command or short tightly related chain for shell semantics or fewer model/tool round trips. Predetermined related observations may share one call; adaptive/result-dependent follow-ups stay sequential. Prefer run_process for one native executable with literal argv. A bounded deterministic Python heredoc fits when one small program expresses a coherent transformation more reliably than many mechanical edits; respect Project/path/permission policy, avoid unauthorized network, inspect diff, validate final source. run_script handles supported program-like languages and does not imply Python. Do not chain independent effects or failure/permission/validation boundaries such as commit, push, deploy, restart. Persistent shell is for same-process state or repeated commands on one named SSH resource. Runner-owned; use run_detached_process to outlive this Runner.",
            run_shell_input_schema,
        ).with_gpt_action_description("Run bounded shell syntax or a tightly related chain. Batch predetermined related observations; keep adaptive/result-dependent follow-ups and independent effect/validation boundaries separate. Prefer run_process for literal argv. Longer work may hand off as the same Job.")
        .with_execution(super::ToolExecutionContract::new(
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Continue already-known Jobs by job_id; do not call list_jobs first. Pass observation_token unchanged as after_observation_token and use newer tokens on follow-ups. Observe 1-8 Jobs with bounded baseline/deltas and isolated errors. No token gives immediate baseline; no wait_secs gives immediate observation. With tokens, bounded wait_secs (max 100) uses wake_on=change by default or wake_on=terminal to coalesce updates. The canonical wait_secs=100,wake_on=terminal continuation is for when useful progress is blocked on terminal outcome; if independent work remains, retain the exact Job continuation and observe later—do not poll for visibility. Terminal/errors wake early; updates do not extend the deadline. Timeout may include changed=true deltas. reset is bounded recovery. Use list_jobs only for lost identity/inventory or unknown_job. Never launches, retries, stops, or subscribes.",
