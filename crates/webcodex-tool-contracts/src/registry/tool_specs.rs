...
pub fn agent_continuation_app_tool_specs() -> Vec<ToolSpec> {
    vec![
        tool_spec(
            ...
            super::input_schemas::agent_wait_state_input_schema(),
        ),
        tool_spec(
            "agent_continuation_bind",
            "App-only bind of one live View to one exact current Durable Agent Endpoint generation. Supply a stable binding_id generated by the View using 128 secure random bits (wc_host_binding_ plus 32 lowercase hex). It is the iframe/process-instance fence, never authority. Re-authorizes communication:manage on every call. Normal first bind requires the current process attachment; after Server restart, recovery remains limited to the exact durable binding fingerprint or a canonical Host ClientWindow continuity identity carried outside tool input. Retry a lost response with the same exact binding_id renews without revoking claims or resetting dispatch; a same-Window refreshed iframe may replace its older fence. A different Host Window, stale generation, expired Endpoint, or foreign principal fails closed. No durable continuation truth is created.",
