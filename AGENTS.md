...
- Design model-facing tool contracts for turn economy: reject semantic ambiguity, authority changes, stale fences, and uncertain effects, but normalize recognized harmless defaults and clamp presentation/resource budgets when doing so cannot change meaning. Do not spend a model turn correcting a server-known mechanical detail.
- Keep one canonical machine representation per fact or follow-up. Model-facing tool compatibility requires a named concrete consumer; historical fields, aliases, and duplicate projections are not retained merely because an older implementation emitted them. See [`docs/agent/tool-contract-guidelines.md`](docs/agent/tool-contract-guidelines.md).
